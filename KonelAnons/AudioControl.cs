...
namespace KonelAnons
{
    ...
    public static class AudioUtilities
    {
        ...
        public static void SetApplicationVolume(int pid, float level, Guid containerId)
        {
            ISimpleAudioVolume volume = GetVolumeObject(pid);
            if (volume == null)
                return;
            ...
            volume.SetMasterVolume(level / 100, containerId);
            Marshal.ReleaseComObject(volume);
        }

        public static void SetApplicationMute(int pid, bool mute,Guid containerId)
        {
            ISimpleAudioVolume volume = GetVolumeObject(pid);
            if (volume == null)
                return;
            ...
            //Guid guid = Guid.Empty;
            volume.SetMute(mute, containerId);
            Marshal.ReleaseComObject(volume);
        }
        ...
        private static ISimpleAudioVolume GetVolumeObject(int pid)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                IAudioSessionControl ctl2;
                sessionEnumerator.GetSession(i, out ctl2);
                int cpid;
                var ff = ctl2 as ISimpleAudioVolume;
                ctl = ctl2 as IAudioSessionControl2;
